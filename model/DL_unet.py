...
class Down(nn.Module):
    ...
    def forward(self, x):
        ...
        return x
class Dali_Down(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(Dali_Down, self).__init__()
        self.maxpool = nn.MaxPool2d(kernel_size=2, stride=2)
...
class Dali_Up(nn.Module):
    def __init__(self, in_channels, out_channels, bilinear=True):
        super(Dali_Up, self).__init__()
        if bilinear:
            self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
...
class DL_UNet(nn.Module):
    def __init__(self, in_channels,
        ...
        self.down1 = Dali_Down(base_channels, base_channels*2)
        self.down2 = Dali_Down(base_channels*2, base_channels*4)
        ...
        self.up3 = Dali_Up(base_channels * 4, base_channels * 2 // factor, bilinear)
        self.up4 = Dali_Up(base_channels * 2, base_channels, bilinear)
