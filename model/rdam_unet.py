...
class DWRDAM_UNet(nn.Module):
    def __init__(self, 
        ...
        self.base_channels = base_channels
        # 编码器
        self.inconv = DWDoubleConv(in_channels, base_channels)
        self.acpn0 = AMSFN(base_channels)
        self.msaf1 = MDAM(base_channels) 

        self.down1 = DWResD_Down(base_channels, base_channels*2)
        self.acpn1 = AMSFN(base_channels*2)       
        self.msaf2 = MDAM(base_channels*2)
        ...
        # 解码器
        self.up1 = DWResD_Up(base_channels * 8 , base_channels * 4, bilinear=bilinear)
        self.acpn5 = AMSFN(base_channels*4)
        self.decoder_dropout1 = nn.Dropout2d(0.15 if p!=0 else 0)   

        self.up2 = DWResD_Up(base_channels * 4, base_channels * 2 , bilinear=bilinear)
        self.acpn6 = AMSFN(base_channels*2)    
        self.decoder_dropout2 = nn.Dropout2d(0.1 if p!=0 else 0) 

        self.up3 = DWResD_Up(base_channels * 2, base_channels, bilinear=bilinear)
        self.acpn7 = AMSFN(base_channels)
        self.decoder_dropout3 = nn.Dropout2d(0.05 if p!=0 else 0) 

        self.up4 = DWUp(base_channels, base_channels, bilinear=bilinear)
        self.acpn8 = AMSFN(base_channels)
        # 输出层
